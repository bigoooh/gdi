...
def get_data(train_path,dev_path):
    with open(train_path) as f:
        data=f.readlines()
        X_train,labels_train=[d.split('\t')[0] for d in data],[d.split('\t')[1].replace('\n','') for d in data]
...
def create_model(conv_units=128, lr=1e-4,drop=0.2,conv_layers=3):
    ...
    to_merge=[]
    for i in range(2,8):
        ch_conv=Conv1D(256,3,activation='relu')(ch_embed)
    ...
    ch_ouput=Dense(4,activation='softmax')(mrg)
...
y_train=np.array([to_categorical(y,4) for y in y_train])

y_dev=np.array([to_categorical(y,4) for y in y_dev])
